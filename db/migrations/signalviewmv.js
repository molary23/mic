...
const original_query =
  "SELECT Signals.id AS signalid, Signals.signaloption, Signals.takeprofit, Signals.stoploss, Signals.startrange, Signals.endrange, Signals.pip, Signals.status, Signals.createdAt, Signals.updatedAt, Currencies.currency, CONCAT(Profiles.firstname, ' ' , Profiles.lastname) AS providername, Users.email AS provideremail FROM Signals INNER JOIN Users ON Users.id = Signals.UserId INNER JOIN Profiles ON Signals.UserId = Profiles.UserId INNER JOIN Currencies ON Signals.CurrencyId = Currencies.id  ";
